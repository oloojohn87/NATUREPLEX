$(document).ready(function()
{
    ...
    $.fn.H2M_ProbeGraph = function(dictionary) 
    {
        ...
        var protocol_name = dictionary.protocol_name;
        var protocol_description = dictionary.protocol_description;
        var question_description = dictionary.question_description;

        var min_days = dictionary.min_days;
        var max_days = dictionary.max_days;
        if (min_days < 1 && max_days < 1) var no_path = 1; else var no_path = 0;
        if (max_days == 0) max_days = 30;

        var start_value = dictionary.min_value;
        var end_value = dictionary.max_value;
        var scalemax = dictionary.max_scale;
        var scalemin = dictionary.min_scale;
        var inverted = dictionary.inverted;
        ...
        container.innerHTML = '\
            <div id="graph_upper_bar" style="width: '+width+'px; height: 35px; background-color: white; text-align: center; color: #FFF; font-size: '+probe_data_font_size+'px; font-family: Helvetica, sans-serif; padding-top: '+Math.floor((height * 0.15) * 0.1)+'px;">\
                <div id="probe_name" style="font-family:Lato; font-size:14px; color:#777; float:left; margin-left:10px; width:200px; text-align:left;">'+protocol_name+'</div>\
                ...
                <div class="question_label" id="probe_question_label_'+probe_id+'" style="font-family:Lato; font-size:12px; color:#777; float:right; margin-right:10px;">'+title+'</div>\
            ...
            <div id="graph_bottom_bar" style="width: '+width+'px; height: 25px; background-color:#e0e0e0;">\
                    ...
                    <div style="float:left; width: 150px; height: 30px; margin-top: 2px; margin-left:50px;">\
                        <span style="font-family:Arial; font-size:10px; color:grey; margin-left:5px;">Show Predicted Path</span>\
                        <input id="pathToggle" id2="'+probe_id+'" class="path_toggle h2m-small-toggle h2m-small-toggle-round" type="checkbox" checked="checked"/>\
                        <label for="pathToggle" data-on="Yes" data-off="No" style="float: left; margin-top:2px;"></label>\
                    </div>\
                    <div style="float:left; width: 150px; height: 30px; margin-top: 2px; margin-left:50px;">\
                        <span style="font-family:Arial; font-size:10px; color:grey; margin-left:5px;">Show Alerts</span>\
                        <input id="alertToggle" id2="'+probe_id+'" class="alert_toggle h2m-small-toggle h2m-small-toggle-round" type="checkbox" checked="checked"/>\
                        <label for="alertToggle" data-on="Yes" data-off="No" style="float: left; margin-top:2px;"></label>\
                    </div>\
            </div>';   
        ...
        for (var p = parseInt(scalemin); p < parseInt(scalemax); p+=parseInt(step_axis))
        {
            ...
            var v_axis_label_text = new Konva.Text({
              x: trans_x(0)-30-30,
              y: trans_y(p)-5-3,
              text: H2M_ProbeGraphLabel(p, data_values).substr(0,6),
              fontSize: 10,
              fontFamily: 'Lato',
              opacity: 0.4,    
              fill: 'black'
        ...
        */
        ...
        if (no_path == 0)
        {
                ...
                if (inverted == 1) upper_limit = scalemin; else upper_limit = scalemax;    
                var goal_points = [trans_x(min_days), trans_y(end_value),trans_x(max_days), trans_y(end_value),trans_x(max_days), trans_y(upper_limit),trans_x(min_days), trans_y(upper_limit)];
                ...
                if(inverted == 0){
					var display_inverse_switcher_upper = 'GOAL AREA';
					var display_inverse_switcher_lower = 'FAIL AREA';
				}else{
					var display_inverse_switcher_upper = 'FAIL AREA';
					var display_inverse_switcher_lower = 'GOAL AREA';
				}

                var goal_area_width = trans_x(max_days) - trans_x(min_days);
                if (inverted == 1) var goal_area_height = Math.abs(trans_y(end_value)-trans_y(scalemin));
                else var goal_area_height = Math.abs(trans_y(scalemax) - trans_y(end_value));

                var goal_area_label = new Konva.Text({
                  text: display_inverse_switcher_upper,
                  fontSize: 12,
                  fontFamily: 'Lato',
                  opacity: 0.5,    
                  fill: 'black'
                ...
                if (inverted == 1) goal_area_label.setY (trans_y(end_value) + (goal_area_height/2) - (goal_area_label.getTextHeight()/2)); 
                else  goal_area_label.setY (trans_y(end_value) - (goal_area_height/2) - (goal_area_label.getTextHeight()/2));

                if (inverted == 1) upper_limit = scalemax; else upper_limit = scalemin;    
                var fail_points = [trans_x(min_days), trans_y(end_value),trans_x(max_days), trans_y(end_value),trans_x(max_days), trans_y(upper_limit),trans_x(min_days), trans_y(upper_limit)];
                ...
                if (inverted == 1) fail_area_height = Math.abs(trans_y(scalemax)-trans_y(end_value));
                var fail_area_label = new Konva.Text({
                  text: display_inverse_switcher_lower,
                  fontSize: 12,
                  fontFamily: 'Lato',
                  opacity: 0.5,    
                  fill: 'black'
                ...
                if (inverted == 1) fail_area_label.setY (trans_y(end_value) - (fail_area_height/2) - (fail_area_label.getTextHeight()/2)); 
                else  fail_area_label.setY (trans_y(end_value) + (fail_area_height/2) - (fail_area_label.getTextHeight()/2));


                if (inverted == 1) upper_limit = scalemin; else upper_limit = scalemax;    
                var overperform_points = [trans_x(0), trans_y(end_value),trans_x(min_days), trans_y(end_value),trans_x(min_days), trans_y(upper_limit),trans_x(0), trans_y(upper_limit)];

                var overperform_area = new Konva.Line({
                  points: overperform_points,
                  fill: '#bc7b00',
                  stroke: '',
                  strokeWidth: 0,
                  opacity: 0.05,
                ...
                if (inverted == 1) var overperform_area_height = Math.abs(trans_y(end_value) - trans_y(scalemin));
                else var overperform_area_height = Math.abs(trans_y(scalemax) - trans_y(end_value));
                ...
                if (inverted == 1) overperform_area_label.setY (trans_y(end_value) + (overperform_area_height/2) - (overperform_area_label.getTextHeight()/2));
                else overperform_area_label.setY (trans_y(end_value) - (overperform_area_height/2) - (overperform_area_label.getTextHeight()/2));
            ...
            //    label.textContent = H2M_ProbeGraphLabel(sample_data[k].value, data_values);
                ...
                function trans_y (val)
                {
                    ...
                    if (scalemax != 0) res = draw_height - ((val-scalemin) * draw_height / (scalemax-scalemin)) + margin_top;
                    return res;
                }
        ...
        var scale_on = 1;
        $(".path_toggle").on('change', function(e)
        {    
            var thisprobe = $(this).attr('id2');
            //if (no_path == 1)  swal("No evolution path!", ("Please edit the probe and create an evolution path for this patient."), "warning");;
            if (scale_on == 1) { 
                group_paths.hide();
                scale_on = 0;  
            }
        ...
        var alerts_on = 1;
        $(".alert_toggle").on('change', function(e)
        {      
            var thisprobe = $(this).attr('id2');
            if (alerts_on == 1) { 
                group_alerts.hide();
        ...
        var first_day = moment(sample_data[0].date.substr(0, 10)).subtract(1, 'days');
        var last_day = moment(sample_data[sample_data.length-1].date.substr(0, 10));
        ...
        {
            var init_day = moment(sample_data[0].date.substr(0, 10));
            var this_day = moment(probe_alerts[n].date.substr(0, 10));
