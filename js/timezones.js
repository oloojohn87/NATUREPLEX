...
function determine_timezone() {
    ...
    return {'timezone' : olson.timezones[tz_key], 'key' : tz_key, 'dst' : timezone_key_info.dst}
}
...
function get_timezone()
{
    ...
    {
        response = tz_info.timezone.olson_tz;
    }
    return response;
}
function get_timezone_offset()
{
    ...
    {
        response = tz_info.timezone.utc_offset;
    }
    if(response != 'none' && tz_info.dst == 1)
    {
        var hour = parseInt(response.substr(0, 3));
        hour += 1;
        if(hour > 12)
        {
            hour = -12;
        ...
        hour_str += Math.abs(hour).toString();
        hour_str += response.substr(3, response.length - 3);
        response = hour_str;
