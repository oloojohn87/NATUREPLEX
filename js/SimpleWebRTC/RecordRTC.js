...
// convert video and audio streams into a single file
...
{
    var fileType = 'video';
    var fileName = appointment_id.toString() + "_doctor.mp4";
    ...
    xhr('../save_rtc.php', formData, function (fileURL){alert(data);});
}
...
function convertStreams(videoBlob, audioBlob) {
    ...
    worker.onmessage = function(event) {
        ...
        if (message.type == "ready") {
            console.log('<a href="ffmpeg_asm.js" download="ffmpeg-asm.js">ffmpeg-asm.js</a> file has been loaded.');
            workerReady = true;
            if (buffersReady)
                postMessage();
            ...
            console.log('<a href="ffmpeg_asm.js" download="ffmpeg-asm.js">ffmpeg-asm.js</a> file received ffmpeg command.');
    ...
    var postMessage = function() {
        ...
        worker.postMessage({
            type: 'command',
            arguments: ['-i', 'video.webm', '-i', 'audio.wav', '-c:v', 'mpeg4', '-c:a', 'vorbis', '-b:v', '64k', '-b:a', '12k', '-strict', 'experimental', 'output.mp4'],
            files: [
                {
