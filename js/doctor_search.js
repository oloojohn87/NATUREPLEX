...
$("#filter-catapult").on('click', function()
{
...
$('.doctor_row').live('click', function()
{
    ...
    else
    {
        ...
        $(this).animate({height:'70px'},  {duration: 500, queue: false});
        $(this).parent().animate({height:'86px'}, {duration: 500, queue: false});
    }
});
$(".doctor_action_button").live('click', function()
{
	var doctor_price_holder = this.id;
	//alert(doctor_price_holder);
	swal({   title: "You will be charged!",   text: "By continuing you understand that the credit card we have on file will be charged $"+doctor_price_holder+".",   type: "success",   confirmButtonText: "Ok" });
...
function get_doctors()
{
    ...
    var grant_access = '';
    if($("#filter-catapult").attr("data-on") == "true" && personal_doctor_search == 0)
    {
        grant_access = 'CATA';
    }
    var telemed_only = 0;
    ...
    $.post("getTelemedDoctors.php", {grant_access: grant_access, starting_point: doctors_next_result, order: order_column, asc: order_direction, timezone: get_timezone_offset(), online_only: online_only, telemed_only: telemed_only, search_term: $("#search_bar").val(), speciality: sp, location: loc, personal_doctor_search: personal_doctor_search, pat_id: $("#USERID").val(), tag: doctor_search_tag}, function(data,status)
    {
        ...
        if(info['tag'] == doctor_search_tag)
        {
            ...
            for(var i = 0; i < docs.length; i++)
            {
                ...
                html += '</div>';
                html += '<div class="doctor_consult_price" style="margin-top:45px;font-size:75%;">';
                if(parseInt(docs[i]['consult_price']) > 0){
					var price_holder = (parseInt(docs[i]['consult_price'])+1200) / 100;
				}else{
					var price_holder = 12;
				}
                html += 'Consultation Price: $'+price_holder;
                html += '</div>';
                ...
                if(personal_doctor_search == 0)
                {
                    if(docs[i]['available'])
                    {
                        html += '<button class="doctor_action_button" id="'+price_holder+'">Call Now</button>';
                    }
                    if(docs[i]['telemed'] == 1)
                    {
                        ...
                        {
                            html += '<button class="doctor_action_button" id="'+price_holder+'">Schedule</button>';
                        }
                    }
                }
                else
